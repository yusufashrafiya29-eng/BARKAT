...
def run():
    with engine.connect() as conn:
        ...
        try:
            ...
            print("OK: Column 'is_approved' added.")
        except Exception as e:
            print(f"WARN: Column may already exist: {e}")

        # Auto-approve all existing OWNER accounts
        conn.execute(text(
            "UPDATE users SET is_approved = TRUE WHERE role = 'OWNER'"
        ))
        print("OK: All existing OWNERs approved.")
        ...
        print("OK: All previously verified staff auto-approved.")
